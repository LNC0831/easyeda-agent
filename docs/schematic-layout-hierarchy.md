...
> 2026-09-14 状态订正：以下保留 2026-08 的设计与分工记录，不是当前执行指令。
> 新设计和布局修复遵守 [数据驱动架构基准](../skills/easyeda-agent/references/schematic-data.md#数据驱动架构基准)：
> 显式核心/外围所有权 → 区内求解 → 纸张层 → 固定转换/Apply → 数据回读。
> 旧 tidy/move 只用于有源数据同步的存量维护；“逐步回滚”不是事务保证，“netport 永不竖放”
> 不是现行通用规则。旧并行分工表是历史记录，不授权当前会话开 agent 或 worktree。
> 局部 bbox 零重叠不能证明所有权、外围直连或全部图形已覆盖。
