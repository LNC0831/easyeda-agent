...
线岛、渲染和执行回读的接触语义；完成升级后的新会话门禁和现场回读前不能称现场已覆盖。异网端点/T/重叠及穿引脚、
...
覆盖范围为准：外围语义归属尚须显式输入，proximity 评分不等于所有权硬门，单一 gate
...
累计分支上限为每区成员数 × 128，并夹在 128..1024。先用分支额度前半段定向调整
...
再单件及双件），每次记录配额、消耗和失败原因，不重置总预算；原姿态预算为
`min(总预算,max(20000,总预算/2))`，避免把旧默认预算内的合法方案人为截断。
...
目标页的完整实例保全门禁。
此段是实现及验收契约；开发包安装不等于现场已通过，以新会话版本门禁和实际回读为准。
