	...
	for i, c := range p.Placements {
		for _, box := range c.TextBBoxes {
			if !plBoxValid(box) || !boxInside(box, sheet) {
				return schObstruction("text-bounds", fmt.Errorf("%s invalid/out-of-sheet text bbox", c.Designator), c.Designator)
			}
			for j, other := range p.Placements {
				if j != i && boxesGapOverlap(box, other.BBox, 5) {
					return schObstruction("text-body", fmt.Errorf("text collision: %s label / %s body", c.Designator, other.Designator), c.Designator, other.Designator)
				}
				if j < i {
					for _, b := range other.TextBBoxes {
						if boxesGapOverlap(box, b, 5) {
							return schObstruction("text-text", fmt.Errorf("text collision: %s / %s labels", c.Designator, other.Designator), c.Designator, other.Designator)
						}
					}
				}
			}
			...
			for _, f := range p.Flags {
				for _, b := range schTerminalMarkerBoxes(f) {
					if boxesGapOverlap(box, b, 5) {
						return schWireObstruction(p, "marker-text", fmt.Errorf("text collision: %s marker / %s label", f.Net, c.Designator), []string{f.Net}, c.Designator)
					}
				}
			}
		}
