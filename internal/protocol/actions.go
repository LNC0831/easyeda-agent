...
func AllActions() []ActionSpec {
	return []ActionSpec{
		...
		},
		{
			...
			Phase:       2,
			NeedsWindow: true,
			Description: "Read the current board outline: outline (polyline 类型=板框 count) + legacy segment/arc counts + bounding box (from the polyline's rendered extent) + the TRUE polygon `points` (#167: CENTER-LINE vertices/sampled arcs = the milled board edge; the bbox is the rendered extent and includes line width). Verified ARC sources `[ARC,signedSweepDegrees,endX,endY]` are sampled at <=2 degrees (#215). With multiple parseable rings, points are returned only when one unique largest ring contains every other ring; disjoint/equal/unsupported geometry remains ambiguous and degrades to bbox.",
