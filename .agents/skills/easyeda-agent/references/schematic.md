...
先保留原生告警原文，再区分不同名称冲突和相同名称重复。原生格式中同一导线可挂多条 NET 属性；只看 wire.net 或对网名做集合去重会丢掉重复属性证据。需检查父图元 ID、属性 ID、key 和原始 value，排除空白/不可见字符。不要把 API 重建的“无冲突”当作原生警告已消除。修复应在独立副本中通过 typed 能力完成，前后核对真实网络端点、参数和身份并保存重载复查原生 DRC；缺少属性级接口时记录能力缺口，不直接篡改归档或忽略告警。

### 原生重复命名告警的副本修复

当原生规则和原始属性证据共同确认同一导线重复 NET 属性，先保留源快照并在独立副本建立参数化修复计划。若没有受支持的属性删除接口，不调用标为 internal 的 SDK 方法；可通过已有 typed primitive delete / wire create 重建最小受影响线段，保留端口和器件，仅在现有端口已提供网名时省略重复显式命名。写前核对实时对象身份，删除后核对结果，再按保存的唯一几何线段重建。逐步留证并保存重载，对账器件身份/参数/真实引脚、网络端点及导线覆盖，复查原生严格 DRC。宿主在端口处拆分线段时，可比较共线区间合并后的覆盖，不能只比线段数量，也不能忽略几何差异。此方法仅在根因和最小重建范围已确认时适用；不得据此重画整个工程或掩盖其他规则。
