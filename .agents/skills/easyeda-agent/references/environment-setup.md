...
health 成功只证明本地 daemon/连接器链路可达，不证明云端网络、账号登录或目标工程权限有效。云端工程打开失败时记录宿主版本、模式、原始返回及当前身份，先核对网络和原账号登录状态，再有界重试；不要仅凭文件模式提示自动切换离线模式。桌面 3.2.149 的一条“全在线模式不支持 eprj/eprj2”提示也可由未识别登录且目标非公开的检查分支触发，不能据此认定正在导入文件。

侧载连接器更新若出现扩展管理器闪退，可在保存文档后先禁用连接器的外部交互，再卸载/导入、恢复交互并重启客户端。这是现场观察到的规避步骤，不是已证明的生命周期修复；更新后读取真实连接数与版本。

## 图像输出和工具缓存

导图的显式 --out 是交付文件路径，不保证关闭 .easyeda/artifacts 缓存。需要限制输出边界时，先确定 CLI/MCP 的 cwd 与项目根定位，分别声明交付目录、缓存目录、审计日志和 Trace store；盘点缓存变化并在交付目录保存哈希索引。不要通过删除缓存证据或追认任意写入来满足验收。
